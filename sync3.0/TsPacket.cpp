...
TsPesHeader::TsPesHeader(unsigned short int offset, unsigned char bytes[188], bool _payload_started)
{
	...
	{	
		...
		if(packet_start_code_prefix == PACKET_START_CODE_PREFIX && payload_started)
		{
			isPesStarted = true;
			...
			if(pts_dts_flag == pts_only || pts_dts_flag == pts_and_dts)
			{
				...
				if(write_input_to_file_flag)
				{
					...
					//writeDebugMessage(to_str(pid) + "_pts", to_str(pts_data1) + " " + to_str(pts_data2) + " " to_str(pts_data3) + " " to_str(pts_data4) + " " to_str(pts_data5), ERROR);
					//writeDebugMessage(to_str(pid) + "_pts", to_str(PTS), ERROR);
				}
...
void TsPacket::parse()
{
	...
	switch(packet_type)
	{
		...
		case TS_PES_PRIVATE_DATA_PACKET:
			...
			packet_payload = new TsPesPacket(offset, bytes, header->payload_unit_start_indicator);
			break;
		case TS_PCR_PACKET:
		case TS_NULL_PACKET:
...
TsPesPacket::TsPesPacket(unsigned short int offset, unsigned char bytes[188], bool _payload_started)
{
	payload_started = _payload_started;
	header = new TsPesHeader(offset, bytes, payload_started);
	if(header->hasError)
	{
		hasError = true;
	}
