...
std::string getStrValueFromJSON(json_value *value, std::string label)
{
	switch(value->type)
	{
		...
		case JSON_ARRAY:
			...
			{
				getStrValueFromJSON(value, label);
			}
			break;
		case JSON_STRING:
			if(strcmp(value->name, label.c_str()) == 0)
				...
				)
{
	switch(value->type)
	{
		...
		case JSON_ARRAY:
			for (json_value *it = value->first_child; it; it = it->next_sibling)
			{
				parseSyncJSON(it, frequency, bandwidth, code_rate_HP, code_rate_LP, inversion, constellation, transmission_mode, guard_interval, hierarchy_information);
			}
			break;
		case JSON_STRING:
			if(strcmp(value->name, "frequency") == 0)
				*frequency = atoi(value->string_value) * 1000;
			break;
		case JSON_INT:
			if(strcmp(value->name, "bandwidth") == 0)
				*bandwidth = (fe_bandwidth_t)value->int_value;
			else if(strcmp(value->name, "codeRate_hp") == 0)
				*code_rate_HP = (fe_code_rate_t)value->int_value;
			else if(strcmp(value->name, "codeRate_lp") == 0)
				*code_rate_LP = (fe_code_rate_t)value->int_value;
			else if(strcmp(value->name, "inversion") == 0)
				*inversion = (fe_spectral_inversion_t)value->int_value;
			else if(strcmp(value->name, "constellation") == 0)
				*constellation = (fe_modulation_t)value->int_value;
			else if(strcmp(value->name, "transmitMode") == 0)
				*transmission_mode = (fe_transmit_mode_t)value->int_value;
			else if(strcmp(value->name, "guardInterval") == 0)
				*guard_interval = (fe_guard_interval_t)value->int_value;
			else if(strcmp(value->name, "hierarchy") == 0)
				*hierarchy_information = (fe_hierarchy_t)value->int_value;
			break;
		case JSON_FLOAT:
			break;
		case JSON_BOOL:
